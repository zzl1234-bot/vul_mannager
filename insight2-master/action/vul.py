...
class VulAdd(LoginedRequestHandler):
    ...
    def post(self):
        ...
        if int(doc.get('self_rank')) < 0 or  int(doc.get('self_rank')) > 20 :
            return self.write(dict(status = False, msg = '  rank风险值不正确'))
        if _id:
            #add by wuq
            # 管理员， 安全员角色，以及漏洞发起本人可以编辑漏洞
            ...
            #add by wuq 
            #漏洞编辑权限: 本人的话校验状态，非本人则校验角色是否为管理员，安全员
            if int(vul.user_id) == int(self.uid) and u.role_id==4: #渗透人员
                if vul.vul_status not in [10,20,30]: 
                   return  self.write(dict(status = False, msg = '漏洞状态不允许编辑'))
                else:
                    #如果时这三种状态，被打回的情况，那么修改后状态重置
                    doc['vul_status']=10
            else:
                if u.role_id in [1,3]: 
                    #不重置漏洞编辑人
                    doc['user_id']=vul.user_id
                else:
                    return  self.write(dict(status = False, msg = '当前用户不允许编辑'))
            if not vul.audit_time and vul_status in [40, 60]:
                doc["audit_time"] = time.time()
            elif vul_status in [50]:
                doc["notice_time"] = time.time()
            elif vul_status in [20, 30, 35, 60, 65]:
                doc["fix_time"] = time.time()

            Vul.update(**doc). \
                        where(Vul._id == _id). \
                        execute()
            #add by wuq 重新编辑后，就清除意见信息
            VulExtraInfo.update(content = '无').where(VulExtraInfo._id==_id).execute()

            self.write(dict(status = True, msg = '编辑成功'))
        else:
            # add by wuq
            # 为避免影响原有逻辑，所以"漏洞编号"在新增漏洞的时候采用 vul_id 来摆渡以下
            #新建的情况下，增加重复判断
            doc['_id'] = self.get_argument('vul_id', _id)
            vul = Vul.get_or_none(Vul._id == doc['_id'] )
            if vul:
                return self.write(dict(status = False, msg = '请勿重复添加漏洞'))
...
class VulDel(LoginedRequestHandler):
    ...
    def post(self):
        ...
        vul = Vul.get_or_none(Vul._id == _id)
        #add by wuq 
        #漏洞删除权限: 本人的话校验状态，非本人则校验角色是否为管理员，安全员
        if int(vul.user_id) == int(self.uid) and u.role_id==4:
            if vul.vul_status not in [10,20,30]: 
                return  self.write(dict(status = False, msg = '漏洞状态不允许删除'))
        else:
            if u.role_id in [1,3]: 
                pass
            else:
                return  self.write(dict(status = False, msg = '当前用户不允许删除'))
...
class VulGet(LoginedRequestHandler):
    ...
    def get(self):
        ...
        vul = Vul.get_or_none(Vul._id == _id)
        vul_extra = VulExtraInfo.get_or_none(VulExtraInfo._id == _id)
        ...
        if vul:
            ...
            if vul_extra:
                add_string = "复测人：" + vul_extra.retester+"  \n\n"
                vul['vul_repoc'] = add_string + vul_extra.vul_repoc
                vul['vul_repoc_html'] = vul_extra.vul_repoc_html
        else:
            vul = {}
...
class VulList(LoginedRequestHandler):
    ...
    def get(self):
        ...
        if search:
            # mod by wuq
            cond.append((Vul.vul_name.contains(search)) | (Vul._id.contains(search)))
        ...
        for vul in vuls:
            ...
            # add by wuq
            vul_suggestion = VulExtraInfo.get_or_none(VulExtraInfo._id == vul["vul_id"])
            if vul_suggestion:
                vul['suggestion']=vul_suggestion.content
...
class VulMy(LoginedRequestHandler):
    ...
    def get(self):
        ...
        if vul_status == "0":
            ...
            #add by wuq 增加了复测不通过
            cond.append(Vul.vul_status.in_([20, 30, 35,60,65]))
        else:
        ...
        for vul in vuls:
            ...
            if article_id:
                article = Article.get_or_none(Article._id == article_id)
                if article: vul['article_name'] = article.title
        # print(dict(page_index = page_index,total = total,result = vuls))
...
class VulExport(LoginedRequestHandler):
    """
        ...
        direction: 排序方向[asc, desc]ending
        #add by wuq
        doctype: 文档类型  默认空为csv，否则为docx 转出成word格式
        vul_list: 漏洞编号，为空的话则导出全部，否则导出具体漏洞
    """
    #mod by wuq   get方法更改为post方法，传递参数较多
    def post(self):
        doctype = self.get_argument('doctype', None)
        if not doctype or doctype=='csv':
            self.getCSV()  
        elif doctype == 'docx':
            self.getWORD()
    ...
    def getCSV(self):
        ...
        #具体编号 add by wuq
        vul_list = self.get_argument('vul_list',None)
        ...
        if vul_list:
            cond.append(Vul._id.in_(vul_list.split(',')))
        ...
        title = "漏洞编号,漏洞标题,检测批次,风险等级,所属系统,漏洞URL,解决人员,环境类型,漏洞描述".split(",")
        ...
        for vul in vuls[:]:
            risk_desc=''
            if int(vul.self_rank) < 6:
                risk_desc='2 - 低危'
            elif int(vul.self_rank)>=6 and int(vul.self_rank)< 11:
                risk_desc='1 - 中危'
            elif int(vul.self_rank)>=11 :
                risk_desc='0 - 高危'

            env_desc=''
            app = App.get_or_none(App.id == vul.app_id)
            if int(app.status)==30:
                env_desc='1 - 开发环境'
            elif int(app.status)==20:
                env_desc='4 - 准生产环境'
            elif int(app.status)==10:  
                env_desc='3 - 生产环境'

            data = [vul._id,vul.vul_name,"NULL", risk_desc,app.appname,"NULL", app.op_user_id,env_desc,"详见附件,因环境限制，部分安全测试是在测试环境测试的，请开发人员验证生产是否同样存在问题"]
    ...
    def getWORD(self):
        ...
        vuls = Vul.select(Vul, User).join(User, JOIN.LEFT_OUTER, on=(Vul.user_id == User.id,)).where(*cond).order_by(Vul.app_id,Vul._id)

        md_filename = "/tmp/VUL_{}.md".format(str(datetime.now()))
        with open(md_filename,'w+') as f:
            tmpApp=""
            for vul in vuls[:]:
                app = App.get_or_none(App.id == vul.app_id)
                if tmpApp != app.appname:
                    f.write("# %s\n" % app.appname)
                tmpApp=app.appname
                ...
                f.write("## [%s]%s\n" % (risk_desc.strip(),vul.vul_name.strip()))
                f.write("- 漏洞编号:%s\n" % vul._id.strip())
                f.write("- 风险等级:%s\n" % risk_desc.strip())
                f.write(vul.vul_poc)
                f.write('  \n')
                vulextra=VulExtraInfo.get_or_none(VulExtraInfo._id == vul._id)
                if vulextra:
                    f.write("- 复测结果:")
                    if vulextra.vul_repoc:
                        f.write(vulextra.vul_repoc)
                    f.write('  \n')
                f.write('  \n')

        word_file = "/tmp/VUL_{}.docx".format(str(datetime.now()))
        example_file=os.getcwd()+'/upload/vul_example.docx'
        pypandoc.convert_file(md_filename, 'docx', outputfile=word_file,extra_args=['-V', 'geometry:margin=1.5cm'])

        #add by wuq调整下word中图片格式，没办法玩不转pandoc
        ...
        file_arr.append(word_file)
        
        #----------- 以下单独生成word附件 ------
        for vuln in vuls:
            app = App.get_or_none(App.id == vuln.app_id) 
            risk_desc=''
            if int(vuln.self_rank) < 6:
                risk_desc='低危'
            elif int(vuln.self_rank)>=6 and int(vuln.self_rank)< 11:
                risk_desc='中危'
            elif int(vuln.self_rank)>=11 :
                risk_desc='高危'

            tmpfile="/tmp/"+vuln._id.strip(" /")+".md"
            with open(tmpfile,'w+') as ff:
                ff.write("# %s\n" % app.appname)
                ff.write("## [%s]%s\n" % (risk_desc.strip(),vuln.vul_name.strip()))
                ff.write("- 漏洞编号:%s\n" % vuln._id.strip())
                ff.write("- 风险等级:%s\n" % risk_desc.strip())
                ff.write(vuln.vul_poc)
                ff.write('  \n')
                vulextra=VulExtraInfo.get_or_none(VulExtraInfo._id == vul._id)
                if vulextra:
                    ff.write("- 复测结果:")
                    if vulextra.vul_repoc:
                        ff.write(vulextra.vul_repoc)
                    ff.write('  \n')
                ff.write('  \n')

            tmpwordfile="/tmp/"+vuln._id.strip(" /")+".docx"
            pypandoc.convert_file(tmpfile, 'docx', outputfile=tmpwordfile,extra_args=['-V', 'geometry:margin=1.5cm'])
            doc=Document(tmpwordfile)
            for s in doc.inline_shapes:
                r=s.height/s.width
                s.width=Cm(14.65)
                s.height=Cm(14.65*r)
                doc.save(tmpwordfile)
            os.remove(tmpfile)
            file_arr.append(tmpwordfile)

        zfilename="vul_export.zip"
...
#            self.set_header('Content-Type', 'application/octet-stream')
#            self.set_header('Content-Disposition', 'filename=vul_export.docx')
#            self.write(f.read())
        with open(zfilename, 'rb') as f:
            self.set_header('Content-Type', 'application/octet-stream')
            self.set_header('Content-Disposition', 'filename='+zfilename)
            self.write(f.read())
...
# add by wuq
# 漏洞状态从pms同步（使用文件）
@url(r"/vul/vulsync", needcheck = False, category = "漏洞")
class VulSync(LoginedRequestHandler):
    """
    ...
    def post(self):
        ...
        if u.role_id in [1,3]: 
            ...
            return  self.write(dict(status = False, msg = '当前用户不允许同步状态'))

        xlsfile = self.request.files['xlsfilename'][0]
        filename = xlsfile.get('filename')

        if filename.split(".")[-1].lower() not in ["xls"]:
            return self.write(dict(status = False, msg = "只允许xls格式"))
            
        body = xlsfile.get('body')
        r=''
        try:
            excel=xlrd.open_workbook(filename=None,file_contents=body)
            sheet1 = excel.sheets()[0]   
            nrows=sheet1.nrows
            updateCount=0
            for row in range(nrows):
                vul_id=str(sheet1.cell(row,1).value).strip()

                insight_vul = Vul.get_or_none(Vul._id == vul_id)
                if not insight_vul:
                      continue

                vul_process_status=str(sheet1.cell(row,6).value).strip()
                vul_status=str(sheet1.cell(row,7).value).strip()
                new_vulstatus=insight_vul.vul_status
                if vul_status == '已修复':
                    if vul_process_status == '关闭' :
                       if insight_vul.vul_status != 60:
                           new_vulstatus=60
                    else:
                       new_vulstatus=55
                elif vul_status =='不修复':
                    if vul_process_status == '关闭':
                       if insight_vul.vul_status != 20:
                           new_vulstatus=20
                elif vul_status == '已挂起':
                    if vul_process_status == '关闭':
                       if insight_vul.vul_status != 35:
                           new_vulstatus=35
                elif vul_status == '未解决':
                    new_vulstatus=50
                else:
                ...
                if new_vulstatus != insight_vul.vul_status:
                    Vul.update(vul_status = new_vulstatus).where(Vul._id == vul_id).execute()
                    updateCount=updateCount+1
        except Exception as e:
            print(e)
            return self.write(dict(status = False, msg = "状态同步异常"))
        self.write(dict(status = True, msg = "状态同步完成,共{}个漏洞被同步".format(str(updateCount))))
