...
class AuditingIgnore(LoginedRequestHandler):
    ...
    def post(self):
        ...
        # add by wuq
        if len(content) > 0:
            VulExtraInfo.update(content = content).where(VulExtraInfo._id == _id).execute()
...
class AuditingConfirm(LoginedRequestHandler):
    ...
    def post(self):
        ...
        doc = dict(vul_status = status, audit_time = time.time(),audit_user_id = self.uid)
...
class AuditingFixing(LoginedRequestHandler):
    ...
    def post(self):
        ...
        #add by wuq
        vul = Vul.get_or_none(Vul._id == _id)
        if str(vul.user_id) == str(self.uid):
            return self.write(dict(status = False, msg = "已知悉操作不能为漏洞提交人自己"))
        #####################
...
class AuditingRetest(LoginedRequestHandler):
    ...
    def post(self):
        ...
        vul = Vul.get_or_none(Vul._id == _id)
        if str(vul.user_id) == str(self.uid):
            return self.write(dict(status = False, msg = "申请复测操作不能为漏洞提交人自己"))
        #####################
...
class AuditingFixed(LoginedRequestHandler):
    ...
    def post(self):
        ...
        vul_repoc = self.get_argument('vul_repoc', '')
        vul_repoc_html = self.get_argument('vul_repoc_html', '')
         
        vul = Vul.get_or_none(Vul._id == _id)
        user = User.get_or_none(User.id == self.uid)
        if str(vul.user_id) != str(self.uid) and user.role_id != 3:
            return self.write(dict(status = False, msg = "提交复测操作只能由漏洞发起人和安全官"))
        #####################



        # add by wuq
        if len(content) > 0 or len(vul_repoc)>0:
            VulExtraInfo.update(content = content,vul_repoc = vul_repoc, vul_repoc_html = vul_repoc_html,retester=user.username).where(VulExtraInfo._id == _id).execute()
...
#        add by wuq
@url(r"/auditing/refix", category = "审核")
class AuditingReFix(LoginedRequestHandler):
    """
        漏洞复测不通过
        id: 漏洞id
        status: 修复结果状态40/60
    ...
    def post(self):
        ...
        vul_repoc = self.get_argument('vul_repoc', '')
        vul_repoc_html = self.get_argument('vul_repoc_html', '')
        status = int(self.get_argument('status', 65))
        
        
         
        vul = Vul.get_or_none(Vul._id == _id)
        if str(vul.user_id) != str(self.uid) and str(vul.audit_user_id) != str(self.uid): 
            return self.write(dict(status = False, msg = "提交复测操作只能由漏洞发起人和安全官"))
 
        # add by wuq
        if len(content) > 0 or len(vul_repoc)>0:
            VulExtraInfo.update(content = content,vul_repoc = vul_repoc, vul_repoc_html = vul_repoc_html).where(VulExtraInfo._id == _id).execute()
        Vul.update(vul_status = status, fix_time = time.time()).where(Vul._id == _id).execute()

        email_notify_to(_id, title = "漏洞复测仍旧存在", content = content, uid = self.uid)
        notify_message(_id, title = "漏洞复测仍旧存在", content = content, uid = self.uid)
