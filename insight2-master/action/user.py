    ...
    def get(self):
        ...
        if group_id:
            ...
            if group.parent:
                ...
                groupusers = GroupUser.select().where(GroupUser.group_id == group.id)
...
class UserInfo(LoginedRequestHandler):
    ...
    def get(self):
        ...
        group_ids = []
        # 获取当前用户的身份，来显示不同的项目
        role = User.select().where(User.id == self.uid)
        role_ids = [item.role_id for item in role]
        role_list = [1, 3]
        # 获取用户关联组
        ...
        if role_ids.pop() in role_list:
            my_projects_count = Project.select().count()
        else:
            if project_ids:
                cond1.append(Project.id.in_(project_ids))
                # 用来提取project的数量
            my_projects_count = Project.select().where(*cond1).count()
