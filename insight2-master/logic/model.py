...
class Project(BaseModel):
    id = AutoField()
    project_id = CharField(default = id_func)
    project_name = TextField(default = "")
    project_status = IntegerField(default = 0)
    group_id = ForeignKeyField(Group)
    project_date = DoubleField(default = time.time)
    complete_time = DoubleField(default = "")
    project_paw = CharField(default = "")
...
class App(BaseModel):
    ...
    op_user_id = CharField(default = "") # 临时使用，用于存放app的应用负责人  by wuq

class Projectapp(BaseModel):
    id = AutoField()
    project_id = ForeignKeyField(Project)
    app_id = ForeignKeyField(App)
    status = IntegerField(default = 0)
    content = TextField(default = "")
    asset_address = LongTextField(null=True)
    asset_address_html = LongTextField(null=True)
    prod_passed = TextField(default = "")
    test_passed = TextField(default = "")
...
class Vul(BaseModel):
    ...
    audit_user_id     =    IntegerField(default = 0) 
...
#add by wuq
class VulExtraInfo(BaseModel):
    _id = CharField(default = id_func)
    content = TextField(null = True)
    vul_repoc           =    LongTextField(null = True)
    vul_repoc_html      =    LongTextField(null = True)
    retester = CharField(default="")
def init_db():
