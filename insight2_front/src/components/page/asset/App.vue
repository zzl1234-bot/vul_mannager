  ...
  <div>
    ...
    <div class="container">
      ...
      >
        ...
        <el-table-column prop="appname" min-width="120" label="应用名称">
        ...
        <el-table-column prop="apptype" min-width="65" label="类型">
          <template slot-scope="scope">
            <span
              :style="{'color':statusColor(scope.row.apptype) }"
            >{{scope.row.apptype | statusNameFilter("APP_TYPE")}}</span>
          </template>
        </el-table-column>
        ...
        <!-- add by wuq -->
        <el-table-column prop="group_owner" min-width="100" label="负责人">

          <template slot-scope="scope">
            <span class="insight_sensitive">{{scope.row.group_owner}}</span>
          </template>
        </el-table-column>

        <el-table-column prop="sec_owner" min-width="100" label="安全官">
          <template slot-scope="scope">
            <span class="insight_sensitive">{{scope.row.sec_owner_name}}</span>
          </template>
        </el-table-column>
    ...
    <!-- 新建 -->
    <el-dialog title="新建应用" :visible.sync="createVisible" width="50%">
      <el-form ref="createForm" :model="form" label-width="100px" :rules="rules">
        ...
        <!-- mod by wuq -->
        <el-form-item label="负责人" prop="group_owner">
          <el-autocomplete
            v-model="form.group_owner"
            :fetch-suggestions="querySearchAsyncGroupUser"
            placeholder="请输入内容"
            @select="handleSelectGroupUser"
          ></el-autocomplete>
        </el-form-item>
    ...
    <el-dialog title="编辑应用" :visible.sync="editVisible" width="50%">
      <el-form ref="editForm" :model="form" label-width="100px" :rules="editRules">
        ...
        <!-- mod by wuq <el-form-item label="负责人">{{form.group_owner}}</el-form-item> -->
         <el-form-item label="负责人" prop="group_owner">
          <el-autocomplete
            v-model="form.group_owner"
            :fetch-suggestions="querySearchAsyncGroupUser"
...
<script>
import { cron } from "point-vue-cron";
import {
...
export default {
  ...
  methods: {
    ...
    jumpAsset(app) {
        let routes = this.$router.resolve({ path: "/asset", query:{
          app: app.id
        }});
    ...
    querySearchAsync(queryString, cb) {
      ...
      this.$axios
        .get(this.user_list_url, {
          params: {
            search: queryString,
            //add by wuq role_id  强行绑定用户的role_id, 因此角色这块建议不要随便自定义角色
            role_id:"3"
          }
        })
        .then(res => {
    ...
    // add by wuq
    querySearchAsyncGroupUser(queryString, cb) {
      this.$axios
        .get(this.user_list_url, {
          params: {
            search: queryString,
    ...
    // add by wuq
    handleSelectGroupUser(item) {
      this.form.group_owner = item.value;
    },
