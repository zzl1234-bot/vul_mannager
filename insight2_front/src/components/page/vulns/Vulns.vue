  ...
  <div>
    ...
    <div class="container">
      <div class="handle-box">
        <el-button
          ...
          @click="createVisible=true; form={enable:'1',vul_poc:'- url:  \n\n- 功能点:  \n\n- 测试截图:  \n\n- 解决方案:  \n\n'}"
          size="mini"
        >新增漏洞</el-button>
        <el-input
          size="mini"
        ...
        <div style="float:right">
          ...
          <el-button size="mini" icon="iconfont  el-iconCSV1" @click="exportDocument('csv')">导出CSV</el-button>
          <el-button size="mini" icon="iconfont  el-iconCSV1" @click="exportDocument('docx')">导出成WORD</el-button>
          <el-upload
           style="float: right"
           name="xlsfilename"
            ref="upload"
            accept=".xls"
            :action="vulSyncAction"
            :multiple="false"
            :show-file-list="false"
            :on-success="submitUpload"
            :file-list="fileList"
            :auto-upload="true">
            <el-button size="mini" icon="iconfont  el-iconCSV1"  >导入明细同步状态</el-button>
          </el-upload>
          <el-button size="mini" icon="iconfont el-iconCSV1" @click="dataDel()">批量删除</el-button>
        </div>
      </div>
      ...
      >
        ...
        <el-table-column prop="vul_status" label="状态" min-width="70" sortable="custom">
          <template slot-scope="scope">
    ...
    <el-dialog title="新增漏洞" :visible.sync="createVisible" width="75%" :before-close="handleClose">
      <el-form ref="createForm" :model="form" label-width="150px" :rules="rules">
        <!-- add by wuq 使用vul_id摆渡下-->
         <el-form-item label="漏洞编号" prop="vul_id">
          <el-input
            v-model="form.vul_id"
            clearable
            placeholder="请输入漏洞编号"
    ...
    <el-dialog title="审核漏洞" :visible.sync="auditVisible" width="50%">
      <el-form ref="editForm" :model="form" label-width="90px">
        <!-- add by wuq -->
         <el-form-item label="漏洞编号" prop="vul_id">{{form.id}}</el-form-item>
    ...
    <el-dialog title="处理漏洞" :visible.sync="solveVisible" width="50%">
      <el-form ref="editForm" :model="form" label-width="90px">
        ...
        <el-form-item label="选项">
          <el-checkbox v-model="is_report_visible" v-tooltip="'显示漏洞证明编辑器'">复测证明</el-checkbox>
        </el-form-item>
        <el-form-item label="复测证明" v-show="is_report_visible">
          <mavon-editor
            ref="repoc_editor"
            v-model="form.vul_repoc"
            @change="repocChange"
            @imgAdd="$imgAdd_repoc"
          />
        </el-form-item>
      ...
      <div class="block">
        ...
        <el-timeline :reverse="reverse">
          ...
          >
            ...
            <span style="font-weight: bold;">{{activity.username}}</span>
          </el-timeline-item>
        </el-timeline>
      </div>
      <span slot="footer" class="dialog-footer">
  ...
  data() {
    ...
    return {
      ...
      colData: [
        ...
        { title: "编号", istrue: true } //add by wuq
      ],
...
",
  ...
  methods: {
    ...
    is_related_me() {
      console.log("LOCAL STOARGE", localStorage.getItem("__IS_ONLYME__"));
      return JSON.parse(localStorage.getItem("__IS_ONLYME__"));
    },
    isRelatedMeChange(val) {
      console.log(val);
      localStorage.setItem("__IS_ONLYME__", JSON.stringify(val));
      this.getData();
      this.getStatusGroup();
    },
    desensitive() {
    ...
    exportDocument(t){
        ...
        this.vul_list = this.multipleSelection.map(function(item) {
          return item.id;
        });
        //更改为post方法，可能传递的参数会比较多 by wuq
        this.$axios({
          method: "post",
          url: "/api/vul/export" ,
          data: trans_params({
            ...
            vul_list:this.vul_list.join(',').toString(),
            vul_status:'0',
            doctype: t   //by wuq  t表示文档类型 csv 或者 docx
          ...
          if (t == 'csv'){
            ...
            link.setAttribute("download",  "vulns_export.xls");
          }
          else{
            link.setAttribute("download",  "vulns_export.zip");
          }
    ...
    repocChange(value, render) {
		  this.form.vul_repoc_html = render;
	},
	...
	$imgAdd_repoc(pos, $file) {
	  ...
	  this.$axios({
	    ...
	    this.$refs.repoc_editor.$img2Url(pos,  url.data.path);
	  });
	},
