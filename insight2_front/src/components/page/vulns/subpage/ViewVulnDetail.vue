  ...
  <div>
    ...
    <div class="container">
      <div class="block" id="vul_report">
        <el-row>
          <!-- mod by wuq   add _id -->
          <span class="vuln-title insight_sensitive">{{viewvul._id}}-{{viewvul.vul_name}}</span>
       ...
       <hr style="margin-top:20px;margin-bottom:20px;margin-left:30%;margin-right:30%;background-color:#e8eaef;height: 1px;" />
        ...
        <hr style="margin-top:20px;margin-bottom:20px;background-color:#606266;height:1px;" />
        <div style="width:60%;">
          <div v-html="marked(filterXSS(viewvul.vul_repoc)) " class="markdown-body insight_sensitive"></div>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  created() {
    if (this.$route.query.id) {
      ...
      this.$axios
        .get(" /api/vul/get?id=" + this.$route.query.id, {})
        .then(res => {
